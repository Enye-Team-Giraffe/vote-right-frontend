...
export default function ViewStats({ match }) {
    ...
    const maleVote=Object.keys(getCandidatesNameById).map(id=>{
        return  group
                .filter(group=>group['gender']==='male' && group['votedCandidate']===id)
                .map(group=>group['Value'])
                .reduce((current,previous)=>current+previous,0)
    })
    // get the total number of female votes for each candidate
    const femaleVote=Object.keys(getCandidatesNameById).map(id=>{
        return  group
                .filter(group=>group['gender']==='female' && group['votedCandidate']===id)
                .map(group=>group['Value'])
                .reduce((current,previous)=>current+previous,0)
    })
